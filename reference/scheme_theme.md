...
overridden by any `theme()` settings applied directly to the plot. The
default theme (`scheme_theme()`) is applied first, followed by any
specific `theme()` settings, even if `theme()` is added before
`scheme_theme()`.
