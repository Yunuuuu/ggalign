...
It is important to note that both `htanno_group` and `htanno_kmeans` cannot do
...
You may have noticed that the `htanno_group` function does not explicitly set
the `position` argument after `htanno_kmeans`. When adding an annotation, we can
...
In the case of `htanno_kmeans`, the default `set_context` value is `c(TRUE,
FALSE)`, implying that the active context is already set by `htanno_kmeans`. 

### htanno_dendro
`htanno_dendro` class the addition of a dendrogram near the heatmap and and it
...
`htanno_dendro` is capable of drawing plot components. Therefore, it has a
default `set_context` value of `c(TRUE, TRUE)`, we can directly add ggplot
elements after `htanno_dendro` function. 
...
`htanno_dendro` is also capable of performing clustering between groups. This
...
ggheat(small_mat) +
  htanno_group(column_groups, position = "t") +
...
second `htanno_dendro` will follow the group ordering established by the first
...
## active
An alternative method to modify the active context is by adding an `active`
object.
...
you can set `labels` or `breaks` in `ggheat()` function. See `xlabels/ylabels`
