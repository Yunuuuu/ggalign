...
In addition, we can provide the integer index directly in the `order` argument,
in this way, we should wrap the integer index in `I()`.
```{r align_reorder_index}
my_order <- sample(nrow(small_mat))
print(rownames(small_mat)[my_order])
ggheatmap(small_mat) +
    hmanno("l") +
    align_reorder(I(my_order))
```
