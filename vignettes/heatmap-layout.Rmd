...
For raster image post-processing similar to what is done in `ComplexHeatmap`,
you can use `geom_raster_resize()`. This function leverages
`magick::image_resize()` to resize raster images with custom filters.
```{r}
ggheatmap(small_mat, filling = NULL) +
    # Use `magick::filter_types()` to check available `filter` arguments
    geom_raster_resize(aes(fill = value), filter = "Lanczos")
```
