...
This ensures that the appropriate data mapping and visualization layers are
automatically applied. Both `quad_anno()` and `quad_active()` are functions
designed for `quad_layout()`. 
...
actions for all plots in the layout. Additionally, `plot_action()` can be
applied directly to specific plots through the `action` argument in the
`align_*()` functions, or it can be added directly to a plot.
...
heatmap_collect_all_guides +
  ...
  quad_active(action = plot_action(guides = "tb"))
```
...
ggheatmap(small_mat) +
  scale_fill_viridis_c() +
  anno_top(size = unit(30, "mm"), action = plot_action(free_spaces = "l")) +
  align_dendro() +
  scale_y_continuous(
    expand = expansion(),
...
heatmap_collect_all_guides +
  ...
  anno_bottom(action = plot_action(guides = "l", free_spaces = "l"))
```

In `ggheatmap()` and `quad_layout()`, the behavior of the `free_spaces` and
`free_labs` arguments differs from other arguments in `plot_action()` when
