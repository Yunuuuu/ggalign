...
body layer. This reduces the resolution of only the heatmap body, which can
improve performance. You can achieve this by using the `raster_magick()`
function with the `dpi` argument. By default, the `dpi` will match the resolution of
the current device, but specifying a different value can reduce the resolution
of the rasterized heatmap body, potentially improving performance compared to
using the pure `geom_raster()` approach.
```{r}
ggheatmap(small_mat, filling = NULL) +
...
package to apply custom image resizing with filters.
```{r}
ggheatmap(small_mat, filling = NULL) +
...
Note: When using `magick::image_resize()`, you must specify the `geometry`
