...
layout.
```{r heatmap_collect_top_and_bottom_guides}
heatmap_with_four_side_guides +
...
A annotation stack is just a [stack layout](`stack_layout()`), it can contain
multiple plots with `align_*()` functions, these functions also have a
`free_guides` argument to control override the `guides` argument in the
annotation stack layout.
