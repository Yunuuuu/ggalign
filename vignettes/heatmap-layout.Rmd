...
If you want to use `magick` to post-processing the raster image like
`ComplexHeatmap`, Please use `geom_magick()` instead.
```{r}
ggheatmap(small_mat, filling = NULL) +
    # Use `magick::filter_types()` to check available `filter` arguments
    geom_magick(aes(fill = value), filter = "Lanczos")
```
