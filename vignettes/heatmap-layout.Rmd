...
set, `ggheatmap()`/`quad_layout()` treat themselves as the active layout. Since
...
`vignette("layout-customize")` for more details).
...
### `with_position`
Along with the `-` operator, `with_position()` can be used to modify the operated
context in `ggheatmap()`/`quad_layout()`.
...
annotation stack, it defaults to `NULL`.
```{r}
my_plot +
  # Change the active layout to the left annotation
  anno_left() -
...
```{r}
my_plot -
  # Modify the background of all plots in all annotation
