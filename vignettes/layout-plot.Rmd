...
## `align_ggplot`
The `align_ggplot()` function allows you to incorporate a ggplot object into
your layout while maintaining alignment with other plot elements. Unlike
`align_gg()`, which aligns every axis value precisely, `align_ggplot()` focuses
more on layout integration without enforcing strict axis alignment. Here's how
it works: 
```{r}
ggheatmap(small_mat) +
    hmanno("top") +
...
ggheatmap(small_mat) +
    ...
    gplot(ggwrap(rectGrob(gp = gpar(fill = "goldenrod")), align = "full")) +
