...
mappings. When used in a `stack_align()`, you typically do not want to inherit
from the layout matrix and should input a data frame manually. The `data`
argument must be explicitly provided, as it comes after `...`. This is rarely
necessary in a `stack_align()`, and it is more commonly used in a
`stack_free()`, where you usually want to inherit from the layout data frame. 

All annotations in `ggheatmap()` are `stack_align()`.
```{r}
ggheatmap(small_mat) +
    anno_top() +
...
All annotations in `ggside()` are `stack_free()`.
```{r}
