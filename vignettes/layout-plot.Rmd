...
## `align_wrap`
The `align_wrap()` function accepts arbitrary graphic and converts it into a
ggplot object, allowing you to incorporate them into your layout. Note that it
won't align every actual axis value. The `ggwrap()` function serves as an alias
for `align_wrap()`.
```{r}
ggheatmap(small_mat) +
    hmanno("top") +
...
You can also wrap any objects that can be converted into a `grob`. You can take
a grob directly:
```{r}
library(grid)
ggheatmap(small_mat) +
    hmanno("top") +
    # ggwrap will create a `ggplot` object
...
You can take a base plot with formula, `pheatmap`, `ComplexHeatmap`, et al. See
