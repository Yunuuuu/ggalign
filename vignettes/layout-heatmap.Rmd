...
In this example:
 ...
 context and sets it as the active plot in the annotation stack. 
 - `geom_point(aes(color = factor(branch)))` is then added to this active plot
 within the annotation stack, allowing you to include additional visual
 elements.
...
We can use absolute unit to define the size.
```{r}
ggheatmap(small_mat) +
  scale_fill_viridis_c() +
...
All `align_*` function has a `size` argument to control the relative width (left
and right annotation) or height (top and bottom annotation) of the single
