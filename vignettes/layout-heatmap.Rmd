...
By default, we'll add the heatmap layer. If you set `filling = FALSE`, a blank heamtap will be drawn.
```{r}
ggheatmap(small_mat, filling = FALSE)
```

## Heatmap annotations
Heatmap annotations add extra information to the rows or columns of the heatmap.
Annotations can be positioned at the `top`, `left`, `bottom`, or `right` of the
heatmap, and this positioning is referred to as the active context in
`ggheatmap()`. 

By default, `ggheatmap()` does not initialize any active context. You can use
`hmanno()` to set the active context, directing all subsequent additions to this
context. This allows you to customize the heatmap layout or add different types
of plots. An annotation is represented as a `layout_stack()` object, which can
...
annotation in the whole annotation stack.
```{r}
ggheatmap(small_mat) +
  scale_fill_viridis_c() +
  hmanno("l", size = 0.2) +
...
function. Alternatively, you can specify a character vector of axis positions
("t", "l", "b", "r") to indicate which axis titles should remain free from
alignment.
...
All `align_*()` functions include the `free_labs` argument, which, by default,
...
The same with `free_labs` argument, you can control the free_spaces argument for
the individual plot in `align_*()` function.
