...
another more effective ways to handle this in the `maftools object` section):
```{r}
...
Key Features of `geom_draw()`:
 ...
 objects (`grob`).
 - Beyond the `x`, `y`, `xmin`, `xmax`, `width`, and `height` aesthetics, you
 may want to rely solely on mapped aesthetics (`aes()`). Unmapped aesthetics
 will default to ggplot2's behavior, which might not produce the desired
 outcome. And always use `native` unit.
...
```{r eval=can_read_maf, fig.dim = c(10, 7)}
ggoncoplot(laml, n_top = 20, collapse_vars = FALSE, filling = FALSE) +
    geom_subtile(aes(fill = value), direction = "vertical") +
    theme_no_axes("x") +
