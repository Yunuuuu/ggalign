...
We can use `align_reorder()` to set the order.
```{r}
# ComplexHeatmap::Heatmap(mat,
#     name = "mat",
...
ggheatmap(mat) +
    ...
    theme(axis.text.x = element_text(angle = -60, hjust = 0)) +
    hmanno("r", size = unit(20, "mm")) +
    align_reorder(I(as.numeric(gsub("row", "", rownames(mat))))) +
    hmanno("t", size = unit(15, "mm")) +
    align_reorder(I(as.numeric(gsub("column", "", colnames(mat))))) &
    theme(plot.margin = margin())
```
...
Above code will run seriate twice, we can manually extract the ordering index.
```{r}
...
ggheatmap(mat2) +
    ...
    theme(axis.text.x = element_text(angle = -60, hjust = 0)) +
    hmanno("r", size = unit(20, "mm")) +
    align_reorder(I(seriation::get_order(o, 1L))) +
    hmanno("t", size = unit(15, "mm")) +
    align_reorder(I(seriation::get_order(o, 2L))) &
    theme(plot.margin = margin())
```
