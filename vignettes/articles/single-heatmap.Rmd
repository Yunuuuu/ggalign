...
Heatmap can be built into a `patchwork` object, you can use `patchwork` to
arrange them.
```{r}
h1 <- ggheatmap(mat) +
...
plot_grid(
  build_alignpatches(h1),
  build_alignpatches(h2),
  build_alignpatches(h3),
  ncol = 2L
)
```
...
ggheatmap(mat) +
  scale_fill_gradient2(low = "#2600D1FF", high = "#EE3F3FFF") +
  theme(axis.text.x = element_text(angle = -60, hjust = 0)) +
  # For row title, we can use `annotation_custom` to add a Grob
  hmanno("r", size = unit(20, "mm")) +
  align_dendro() +
  ggpanel(facet = FALSE, size = unit(5, "mm")) +
  annotation_custom(grid::textGrob(
    "I am a row title",
    rot = -90, gp = gpar(fontface = "bold")
  )) +
  # For column title can be directly added for specific ggplot
  hmanno("t", size = unit(15, "mm")) +
  align_dendro() +
  ggtitle("I am a column title") +
