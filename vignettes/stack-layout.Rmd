...
alias `ggstack()`. Depend on whether you want to align observations along the
stack. There are two types:
...
horizontal stacks use the `y-axis`. However, heatmap layouts always use the
y-axis for observations. When a heatmap layout is added to a vertical stack
layout, the inherited data is automatically converted to a matrix and transposed
before use. 
```{r}
