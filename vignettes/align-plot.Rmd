...
`align_gg()` always applies a default mapping for the parallel axes of the data
index within the layout. This mapping is `aes(y = .data$.y)` for horizontal
stacking (including left and right heatmap annotation) and `aes(x = .data$.x)`
for vertical stacking (including top and bottom heatmap annotation). For more
...
See `ggplot2 specification` in `align_gg()`/`ggalign()` for the data details.
