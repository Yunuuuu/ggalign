...
S7::method(length, GridUnit) <- function(x) length(x@inner)
...
#' @importFrom S7 convert
S7::method(convert, list(GridUnit, S3_unit)) <- function(from, to) from@inner

S7::method(str, GridUnit) <- function(object, ..., indent.str = " ",
                                      nest.lev = 0, give.attr = TRUE) {
    if (!isTRUE(give.attr)) {
        return(invisible(object))
    }
    attr <- attributes(object@inner)
    attr[["class"]] <- NULL
    attr[["names"]] <- NULL
    if (length(attr) == 0) {
        return(invisible(object))
