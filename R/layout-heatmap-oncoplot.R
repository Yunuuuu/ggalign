...
ggoncoplot.default <- function(data = NULL, mapping = aes(), ...,
    ...
    if (reorder_row) {
        ans <- layout_update(ans, anno_left())
        ans <- layout_update(ans, align_order(row_index, reverse = TRUE))
    }
    if (reorder_column) {
        column_scores <- .memo_order(vec_slice(counts, row_index))
        ans <- layout_update(ans, anno_top())
        ans <- layout_update(
            ans,
    ...
    ans <- layout_update(ans, quad_active())
    if (!is.null(filling)) {
        # we always make sure heatmap body has such action data
        ans <- layout_update(ans, scheme_data(data = pdata))
        ...
        ans <- layout_update(ans, ggplot2::geom_tile(tile_mapping))
    }
    ans
}
