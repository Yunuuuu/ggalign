...
S7::method(ggalign_build, QuadLayout) <- function(x) {
    ...
    context <- prop(x, "context")
    control <- prop(x, "control")
    align_plots(
        !!!.subset(plots, keep),
        design = design,
        ...
        guides = prop(
            schemes_get(prop(context, "schemes"), "scheme_align"),
            "guides"
        ),
        theme = prop(context, "theme")
    ) + prop(control, "titles") + layout_tags(NULL)
}
...
#' @noRd
S7::method(layout_build, QuadLayout) <-
    function(layout, context = NULL, orientation = NULL) {
        data <- layout@data
        row_domain <- ggalign_init(layout@horizontal)
        column_domain <- ggalign_init(layout@vertical)
        if (is.function(data)) {
            cli_abort(c(
        ...
        if (is.null(context) || is.null(orientation)) {
            context <- prop(layout, "context")
        } else {
            context <- ggalign_inherit(
                context, prop(layout, "context"),
                align = prop(orientation, "align")
            )
        }

        # prepare schemes for vertical and horizontal stack layout
        ...
        # read the plot ---------------------------------------
        plot <- prop(layout, "plot")
        p <- prop(plot, "plot")
        control <- prop(plot, "control")

        # set the facets and coord ---------------------------
        p <- align_quad_facet(p, row_domain, column_domain, object_name(layout))
        p <- gguse_linear_coord(
        ...
        # add schemes ----------------------------------------
        p <- ggalign_update(p, context,
            schemes = prop(plot, "schemes"),
            align = "xy"
        ) +
            theme_recycle()
