...
schemes_complete <- function(schemes) {
    ...
    if (!any(keys == "ggalign::scheme_theme")) {
        new_sets <- c(new_sets, schemes_set(schemes, scheme_theme()))
    }
    if (!any(keys == "ggalign::scheme_align")) {
        new_sets <- c(new_sets, schemes_set(
            schemes, scheme_align(waiver(), waiver(), waiver())
        ))
    }
...
default_schemes <- function(data = NULL, th = theme()) {
    ...
    Schemes(scheme_data(data), scheme_theme(th))
}
