...
typical workflow includes: 
 ...
 - Adding plots with `ggalign()` or `ggpanel()`, and then layer additional
 ggplot2 elements such as geoms, stats, or scales. 

For documents of the `r if (mode == "devel") "release" else "development"`
version, please see `r sprintf("<%s>", url)`. 
...
- [Layout Customization](`r sprintf("%s/articles/layout-customize.html", url)`)
- [Layout Plot](`r sprintf("%s/articles/layout-plot.html", url)`)
- [Stack Layout](`r sprintf("%s/articles/stack-layout.html", url)`)
- [Special Syntax for Layout Axis](`r sprintf("%s/articles/special-syntax.html", url)`)
